 ...
 */
package object data {
  sealed trait SMValueW[T] {
    protected def smValue: SMValue[T]
    def underlying = smValue.getValue
  }
  implicit def smValueToW[T](s: SMValue[T]) = new SMValueW[T] {
    override protected lazy val smValue = s
  }

  sealed trait StackMobQueryW {
  ...
  implicit def stackMobQueryToW(q: StackMobQuery) = new StackMobQueryW {
